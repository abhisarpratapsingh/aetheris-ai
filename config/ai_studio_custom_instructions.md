# Production Directives (Google AI Studio Constitution)
...
* **Gemini Model Resilience & Fallback Protocol**: Whenever implementing server-side or client-side Gemini AI features with `@google/genai`:
  1. **Resilient Model Fallback Ladder**: Never hardcode a single model string to execute content generation in a single try. Always wrap `generateContent` or `generateContentStream` calls with an automated fallback ladder ordered by availability and latency:
     ...
     - Resilient Long-tail Fallback: `"gemini-2.0-flash"` / `"gemini-1.5-flash"`
...
* **Database Persistence, Clean Payloads, & Transaction Integrity**: Whenever handling user input, document creation, or AI generation workflows:
  1. **Strict Undefined-Stripping (Zero-Crash Payload Hygiene)**: Before passing any object to database SDKs (Firestore `setDoc`/`updateDoc`, SQL ORMs, MongoDB, etc.), sanitize the payload to strip all `undefined` values (e.g., using a recursive undefined-stripper or `JSON.parse(JSON.stringify(payload))`). Never allow `undefined` properties to reach the database driver.
  2. **Guaranteed Transaction Verification (Input-to-Save Completeness)**: Whenever a user submits an input (prompt, form, reflection, chat, or interaction), the application MUST ensure both the user input AND any generated output are successfully persisted. If user input is received but the save operation or downstream generation fails, the system MUST NOT fail silently.
  3. **Explicit Error Escalation & User Feedback**: Always catch database write rejections and display a clear, accessible error banner or toast in the UI with a "Retry Save" option. Never clear the user's input buffer or reset UI state if the persistence operation has not settled with a confirmed successful write.
...
## 8. Google Maps Platform & Geo-Location Security Directives
* **Objective**: Secure location-aware journal tagging and prevent client API key exposure.
* **Directives**:
  * All Google Maps API calls must be proxied through backend server routes or use HTTP-referrer restricted client keys.
  * Coordinates must be validated (Latitude -90 to 90, Longitude -180 to 180).
  * Location metadata stored in Firestore must be scoped under the authenticated user (`/users/{uid}/journal_entries/{id}/location`).

---

## 9. Antigravity Developer Environment & TDD Directives
* **Objective**: Leverage Antigravity local agent skills (`SKILL.md`) and pre-commit security verification.
* **Directives**:
  * Maintain an official security scanner skill in `.gemini/skills/` to audit code before deployment.
  * Enforce pre-commit git hooks that block any commit containing hardcoded strings matching `AIzaSy`.
